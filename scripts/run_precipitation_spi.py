...
def run(
    ...
    chirps_daily_csv: Path | None = typer.Option(
        None, "--chirps-daily-csv", help="Path to the cached CHIRPS daily CSV (from the precipitation layer)"
    ),
    ...
    build_precipitation_spi_layer(
        ...
        cache_dir=cache_dir,
        out_dir=out_dir,
        chirps_daily_csv=chirps_daily_csv,
        monitor=monitor,
        history=history,
    )
