...
Files are cached under cache/ so only missing years are re-downloaded.
...
def main(
    ...
    cache_dir: Path = typer.Option(REPO_ROOT / "cache", help="Cache directory for raw RETC files"),
    ...
    df, _ = build_heavy_metals_layer(
        city=city, cache_dir=cache_dir, out_dir=out_dir, years=years
    )
