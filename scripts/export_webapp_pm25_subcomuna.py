"""Compatibility entrypoint for the PM2.5 spatial-detail publication.

The legacy helpers below remain only for their offline geometry tests.  The
entrypoint publishes the native study raster as a COG through
``exposome detail``: it is AOI-wide, CRS-agnostic, and never phases tiles by
commune.
"""
...
from exposome.pm25 import PM25_BAND, _annual_pm25_image  # noqa: E402
from exposome.spatial_detail import build_study_detail  # noqa: E402

OUT_PATH = REPO_ROOT / "webapp" / "public" / "data" / "subcomuna" / "pm25.geojson"
...
def _sample_pixels(
    ...
    if not chunks:
        raise RuntimeError("PM2.5 tile sampling returned no pixel rows")
...
def _pixels_from_existing_geojson(path: Path) -> pd.DataFrame:
    ...
    if not rows:
        raise RuntimeError(f"No features found in offline PM2.5 source: {path}")
...
def _infer_axis_step(
    ...
    *,
    ...
    if len(diffs) == 0:
        raise RuntimeError("Unable to infer PM2.5 native grid spacing from sampled points")
...
def _points_to_native_rectangles(
    ...
    if not gdf.geometry.map(lambda geom: isinstance(geom, Point)).all():
        raise RuntimeError("PM2.5 sampling must return point geometries at pixel centers")
...
def main(study: str = "santiago_communes") -> None:
    """Materialize the existing native PM2.5 raster locally; no GEE request."""
    results = build_study_detail(study, indicators=["pm25"], resume=True)
