"""Compatibility entrypoint for native Sentinel-5P NO2 detail.

The published detail is the native S5P tropospheric-column raster in mol/m².
It intentionally does *not* apply a commune-level ERA5 BLH scalar to each
pixel: that would inject an administrative discontinuity into a satellite
product.  Legacy conversion helpers remain only for offline historical tests.
"""
...
from exposome.air_quality import M_NO2, _annual_no2_image, fetch_blh  # noqa: E402
from exposome.spatial_detail import build_study_detail  # noqa: E402

OUT_PATH = REPO_ROOT / "webapp" / "public" / "data" / "subcomuna" / "no2.geojson"
...
def _sample_pixels(
    ...
    if not chunks:
        raise RuntimeError("NO2 tile sampling returned no pixel rows")
...
def _pixels_from_existing_geojson(path: Path) -> pd.DataFrame:
    ...
    if not rows:
        raise RuntimeError(f"No features found in offline NO2 source: {path}")
...
def _infer_axis_step(
    ...
    *,
    ...
    if len(diffs) == 0:
        raise RuntimeError("Unable to infer NO2 native grid spacing from sampled points")
...
def _points_to_native_rectangles(
    ...
    if not gdf.geometry.map(lambda geom: isinstance(geom, Point)).all():
        raise RuntimeError("NO2 sampling must return point geometries at pixel centers")
...
def _no2_column_to_surface_ugm3(column_mol_m2, blh_m):
    """column density [mol/m^2] -> surface concentration [µg/m^3].

    Same formula as exposome.air_quality.build_air_quality_layer step 8:
    C_surface = column * M_NO2 * 1e6 / BLH.
    """
    return column_mol_m2 * M_NO2 * 1.0e6 / blh_m


def _load_commune_blh(
    cfg: dict,
    ...
    """Per-commune BLH scalar [m], reusing the air_quality_satellite cache if present."""
    ...
    if df_blh["blh_mean"].isna().any():
        regional_blh = df_blh["blh_mean"].mean()
        df_blh["blh_mean"] = df_blh["blh_mean"].fillna(regional_blh)
    return dict(zip(df_blh["name"], df_blh["blh_mean"], strict=False))


def main(study: str = "santiago_communes") -> None:
    """Materialize existing native NO2 as a COG; no GEE/BLH processing."""
    results = build_study_detail(study, indicators=["no2"], resume=True)
