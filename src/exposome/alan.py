...
def _annual_radiance_image(cfg: dict[str, Any], start: str, end: str) -> ee.Image:
    ...
    return coll.select(band).mean().rename(band).max(0)
...
def build_alan_layer(
    ...
    rad_csv = cache_dir / f"{city}_alan_radiance_{year}.csv"
    if rad_csv.exists():
    ...
    popw_csv = cache_dir / f"{city}_alan_popweighted_{year}.csv"
    if popw_csv.exists():
    ...
    if df["alan_radiance_pop_weighted"].isna().any():
        missing = df.loc[df["alan_radiance_pop_weighted"].isna(), "name"].tolist()
        df["alan_radiance_pop_weighted"] = df["alan_radiance_pop_weighted"].fillna(
            df["alan_radiance_mean"]
        )
        print(
            f"Warning: filled pop-weighted ALAN for {missing} with area mean radiance"
        )
    ...
    metadata = {
        ...
        "resolution_m": cfg["alan"]["collection"]["scale_meters"],
        "units": "nW/cm^2/sr",
        "sources": {
            "viirs_dnb": {
                "provider": "NOAA/Colorado School of Mines (EOG)",
                "collection": cfg["alan"]["collection"]["id"],
                "band": cfg["alan"]["collection"]["band"],
            },
            "population": {
                "provider": "WorldPop",
                "collection": cfg["alan"]["population"]["id"],
                "year": cfg["alan"]["population"]["year"],
            },
        },
