...
def build_sleep_context_layer(
    ...
    master_csv = out_dir / f"{city}_exposome_master.csv"
    master_geojson = out_dir / f"{city}_exposome_master.geojson"
    if not master_csv.exists():
        raise FileNotFoundError(
            f"Missing {master_csv}. Run scripts/build_master_exposome.py first."
        )
    if not master_geojson.exists():
        raise FileNotFoundError(
            f"Missing {master_geojson}. Run scripts/build_master_exposome.py first."
        )
