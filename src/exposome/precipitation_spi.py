...
def _default_chirps_daily_file(city: str) -> str:
    """Legacy fallback filename, only used if a caller omits chirps_daily_csv.

    The standard `exposome run` pipeline always resolves this from the
    study's configured `chirps_daily_csv` input instead (see layers.yaml).
    """
    return f"{city}_precipitation_chirps_daily_2015_2024.csv"


# --------------------------------------------------------------------------- #
# Data loading
# --------------------------------------------------------------------------- #
def load_chirps_daily(data_dir: Path, daily_csv: Path | None = None, city: str = "santiago") -> pd.DataFrame:
    ...
    """
    path = Path(daily_csv) if daily_csv is not None else Path(data_dir) / _default_chirps_daily_file(city)
    if not path.exists():
...
def fetch_recent_anomaly(
    ...
    ``metric_crs`` must be the study's own local metric CRS (``cfg["crs"]["metric"]``)
    — the ``EPSG:32719`` (Chile UTM 19S) default only applies when the caller omits
    it, which should not happen outside ad-hoc/manual invocations.
    ...
    for _, row in tqdm(
        list(centroids.iterrows()), desc="precipitation_spi monitor", unit="commune"
    ):
        name = row["name"]
...
def build_precipitation_spi_layer(
    ...
    chirps_daily_csv: Path | None = None,
    monitor: bool = False,
    history: bool = False,
) -> pd.DataFrame | None:
    ...
    daily_path = Path(chirps_daily_csv) if chirps_daily_csv is not None else data_dir / _default_chirps_daily_file(city)
    daily = load_chirps_daily(data_dir, daily_path)
    ...
    chirps_mtime = daily_path.stat().st_mtime
    ...
    if monitor:
        anomaly_df = fetch_recent_anomaly(daily, gdf_comm, metric_crs=cfg["crs"]["metric"])
    ...
    # 11. Write outputs.
    base_name = f"{city}_precipitation_spi"
