...
- CASEN 2022 multidimensional poverty (``casen_2022_pobreza.csv``).
- Income-poverty SAE estimates with 95% confidence intervals
  (``pobreza_comunal.csv``, MDSF).
...
DEFAULT_SOURCES = {
    "pobreza_comunal": (
        ...
        "pobreza_comunal/datos_procesados/pobreza_comunal.csv"
    ),
    "casen_ingresos": (
        "https://raw.githubusercontent.com/bastianolea/pobreza_chile/main/"
    ...
    "casen_pobreza": (
        "https://raw.githubusercontent.com/bastianolea/pobreza_chile/main/"
        "casen/datos_procesados/casen_2022_pobreza.csv"
    ),
...
# --------------------------------------------------------------------------- #
def fetch_poverty_communal(
    cache_dir: Path, url: str, region_code: int = 13
) -> pd.DataFrame:
    """Income-poverty rate (SAE) and its 95% CI, plus commune population."""
    dest = _download_csv(url, Path(cache_dir) / "ses_pobreza_comunal.csv")
    df = pd.read_csv(dest, sep=";", low_memory=False)
    df["comuna_code"] = pd.to_numeric(df["codigo"], errors="coerce")
    ...
    out = pd.DataFrame(
        {
            ...
            "poblacion": _to_num(df["poblacion"]).round().astype("Int64").to_numpy(),
            "pobreza_pct": (_to_num(df["pobreza_p"]) * 100).round(2).to_numpy(),
            "pobreza_ci_low": (_to_num(df["limite_inferior"]) * 100).round(2).to_numpy(),
            "pobreza_ci_high": (_to_num(df["limite_superior"]) * 100).round(2).to_numpy(),
        }
    )
...
def fetch_casen_multidim_poverty(
    cache_dir: Path, url: str, region_code: int = 13
) -> pd.DataFrame:
    """Multidimensional poverty rate (``pobreza_multi_p``)."""
    dest = _download_csv(url, Path(cache_dir) / "ses_casen_pobreza.csv")
    df = pd.read_csv(dest, sep=";", low_memory=False)
    if "nivel" in df.columns:
        df = df[df["nivel"] == "comuna"].copy()
    df["comuna_code"] = pd.to_numeric(df["cut_comuna"], errors="coerce")
    ...
    out = pd.DataFrame(
        {
            "comuna_code": df["comuna_code"].to_numpy(),
            "pobreza_multi_pct": (_to_num(df["pobreza_multi_p"]) * 100).round(2).to_numpy(),
        }
    )
    return out.drop_duplicates("comuna_code").reset_index(drop=True)
...
def build_socioeconomic_layer(
    ...
    print("  Fetching CASEN / SAE poverty / census housing indicators...")
    pov = fetch_poverty_communal(cache_dir, sources["pobreza_comunal"], region_code)
    inc = fetch_casen_income_education(cache_dir, sources["casen_ingresos"], region_code)
    multi = fetch_casen_multidim_poverty(cache_dir, sources["casen_pobreza"], region_code)
    ...
    metadata = {
        ...
        "sources": {
            ...
            "income_poverty_sae": {
                ...
                "provider": "MDSF small-area estimation via bastianolea/pobreza_chile",
                "url": sources["pobreza_comunal"],
            },
            "multidimensional_poverty": {
                "vars": {"pobreza_multi_p": "pobreza_multi_pct"},
                "provider": "CASEN 2022 via bastianolea/pobreza_chile",
                "url": sources["casen_pobreza"],
            },
            "housing": {
                "vars": {
