...
def compute_exposome_correlations(
    ...
    merged = mortality_df.merge(master_df, on="name", how="inner", validate="many_to_one")
    rows: list[dict[str, Any]] = []
    ...
    for outcome in outcomes:
        ...
        if subset.empty:
            continue
        y = subset["mortality_rate_age_adjusted_per_100k"]
        if y.isna().all():
            y = subset["mortality_rate_crude_per_100k"]
        y_rank = y.rank(method="average")

        for exposure in exposures:
            if exposure not in subset.columns:
                rows.append(
                    {"outcome": outcome, "exposure": exposure, "status": "missing_exposure"}
                )
                continue
            x = subset[exposure]
            ...
            if int(mask.sum()) < 10:
                rows.append(
                    {"outcome": outcome, "exposure": exposure, "status": f"too_few_pairs:{int(mask.sum())}"}
                )
                continue
