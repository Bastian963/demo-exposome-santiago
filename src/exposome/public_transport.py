...
def _build_grid(communes_metric: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    cells = build_study_access_grid(communes_metric, spacing_m=_GRID_SPACING_M, metric_crs=_CRS_METRIC)
    ...
    return gpd.GeoDataFrame(links.drop(columns="geometry").merge(samples, on="cell_id"), geometry="geometry", crs=_CRS_METRIC)
...
def build_public_transport_layer(
    ...
    if len(communes_metric) != expected_communes:
        raise ValueError(f"Expected {expected_communes} communes, got {len(communes_metric)}")
