...
def assign_grid_to_postal_codes(
    ...
    """
    # Estimate a local metric CRS when the caller does not provide one.
    if metric_crs is None:
        metric_crs = postal_gdf.estimate_utm_crs()
        if metric_crs is None:
            raise ValueError("Could not estimate a metric CRS for postal-code assignment")
    grid_m = grid_gdf.to_crs(metric_crs)
    postal_m = postal_gdf.to_crs(metric_crs)
