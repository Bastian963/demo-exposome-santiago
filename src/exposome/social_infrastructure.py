...
from .osm_fetch import fetch_features_from_local_extract
...
try:
    from . import boundaries, config as _config, population as _population
except ImportError:
    import sys as _sys
    ...
    import exposome.population as _population  # type: ignore[no-redef]
...
def _load_or_fetch_group(
    ...
    if cache_path.exists():
        tqdm.write(f"  {group_name}: loading OSM cache ...")
        return gpd.read_file(cache_path)

    tqdm.write(f"  {group_name}: downloading from OSM ...")
    ...
    tqdm.write(f"    -> {len(gdf)} raw features")
    return gdf
...
def _build_grid(communes_metric: gpd.GeoDataFrame, spacing_m: int, crs: str) -> gpd.GeoDataFrame:
    cells = build_study_access_grid(communes_metric, spacing_m=spacing_m, metric_crs=crs)
    ...
    return gpd.GeoDataFrame(links.drop(columns="geometry").merge(samples, on="cell_id"), geometry="geometry", crs=crs)
...
def _load_population(
    ...
    *,
    ...
    spatial_units: gpd.GeoDataFrame | None = None,
    cache_dir: Path | None = None,
) -> tuple[pd.Series, str]:
    """Load a census denominator, falling back to WorldPop when configured."""
    candidates = [
    ...
    for path, col in candidates:
        ...
        if {"name", col}.issubset(df.columns):
            return df.set_index("name")[col].astype(float), path.as_posix()
    if cfg is not None and spatial_units is not None and cache_dir is not None:
        worldpop_path = Path(cache_dir) / f"{city}_worldpop_population.csv"
        df = _population.load_or_fetch_worldpop(cfg, spatial_units, worldpop_path)
        return df.set_index("name")["pop_total"].astype(float), worldpop_path.as_posix()
    raise FileNotFoundError(
        "Could not find population data and no WorldPop fallback was configured."
    )
...
def build_social_infrastructure_layer(
    ...
    population, population_source = _load_population(
        city,
        out_dir,
        cfg=cfg,
        spatial_units=communes,
        cache_dir=cache_dir,
    )
    population = population.reindex(communes_metric["name"])
    ...
    if raw_cache.exists():
        ...
        # A frozen regional extract collapses the whole group loop into one
        # pass. The groups exist only to keep individual Overpass queries
        # small, and splitting actively costs accuracy here: a venue tagged
        # amenity=community_centre AND leisure=sports_centre comes back from
        # Overpass with both tags whichever group matched it, whereas a
        # per-group extract read would give each copy only its own group's
        # columns and _classify_features would assign one category instead of
        # two. `osm_tags` is the union of every group.
        #
        # `access` is requested via extra_keys, not as a filter: line ~594
        # excludes private/customers/no/permit venues from the curated signal
        # by reading that column, and a column the extract never materialized
        # would silently admit every one of them, inflating social_n_total
        # against every other city in the repo.
        extract = social_cfg.get("osm_extract")
        if extract:
            frames.append(
                fetch_features_from_local_extract(
                    extract,
                    social_cfg["osm_tags"],
                    label=f"social_infrastructure[{city}]",
                    extra_keys=("access",),
                    log=tqdm.write,
                )
            )
        else:
            for group_name, tags in tqdm(
                list(tag_groups.items()), desc="social_infrastructure OSM groups", unit="group"
            ):
                frames.append(
    ...
    low_access_out = diagnostics_dir / f"{base_name}_low_access.csv"
    ...
    metadata = {
        ...
        "outputs": [
            ...
            geojson_out.name,
            meta_out.name,
            low_access_out.relative_to(out_dir).as_posix(),
        ],
    }
