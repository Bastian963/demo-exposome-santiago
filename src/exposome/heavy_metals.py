...
try:
    ...
    from .raw_sources import RawSourceSnapshot, load_source_manifest
except ImportError:
    import sys as _sys
    _sys.path.insert(0, str(Path(__file__).resolve().parents[2] / "src"))
    from exposome import boundaries, config  # type: ignore[no-redef]
    from exposome.raw_sources import RawSourceSnapshot, load_source_manifest  # type: ignore[no-redef]
...
RETC_RAW_VERSION = "ckan-2026-06"
RETC_RAW_RELATIVE = Path("data/raw/mma/retc-air-point-sources") / RETC_RAW_VERSION
...
def _emission_to_kg(val: Any, unit: str | None) -> float:
    ...
    # Order matters: check the longer prefixes (kg, mg) before the short
    # ones (g, t) so we don't confuse e.g. "kg/año" with grams.
    if "kg" in unit_low or "kilo" in unit_low:
        return v  # already kg
    if "mg" in unit_low:
        return v / 1_000_000.0  # milligrams → kg
    if "ton" in unit_low or unit_low.startswith("t/") or "/t" in unit_low:
        return v * 1_000.0  # tonnes → kg
    if unit_low.startswith("g/") or unit_low in ("g", "gr/año", "gr"):
...
def resolve_retc_source_files(
    raw_dir: Path,
    years: list[int],
    *,
    legacy_cache_dir: Path | None = None,
) -> tuple[dict[int, Path], RawSourceSnapshot | None]:
    """Resolve verified durable sources, with an explicit legacy-only fallback."""
    raw_dir = Path(raw_dir)
    manifest_path = raw_dir / "source_manifest.json"
    if manifest_path.is_file():
        snapshot = load_source_manifest(raw_dir, verify=True)
        if snapshot.provider != "mma" or snapshot.dataset != "retc-air-point-sources":
            raise ValueError(
                "Unexpected RETC raw snapshot identity: "
                f"{snapshot.provider}/{snapshot.dataset}"
            )
        by_year = {
            int(asset.year): raw_dir / asset.path
        ...
        if missing:
            raise FileNotFoundError(
                f"RETC raw snapshot {snapshot.version} lacks years {missing}; "
                "create a new immutable snapshot version"
            )
        return {year: by_year[year] for year in years}, snapshot

    if legacy_cache_dir is None:
        raise FileNotFoundError(
            f"Verified RETC source manifest not found at {manifest_path}. "
            "Run scripts/migrations/migrate_retc_raw.py first."
        )

    warnings.warn(
        "Reading original RETC provider files from cache/ is deprecated and is "
        "available only through scripts/run_heavy_metals.py during the migration window.",
        DeprecationWarning,
        stacklevel=2,
    )
    ...
    for year in years:
        candidates = [
            ...
            if (path := Path(legacy_cache_dir) / f"retc_efp_{year}{suffix}").is_file()
        ]
        if len(candidates) != 1:
            raise FileNotFoundError(
                f"Legacy RETC source for {year} not found uniquely in {legacy_cache_dir}"
            )
        resolved[year] = candidates[0]
...
def build_heavy_metals_layer(
    ...
    Reads verified annual RETC source files, filters to the Región Metropolitana,
    ...
    if raw_dir is None:
        raw_dir = repo_root / RETC_RAW_RELATIVE
    ...
    source_files, source_snapshot = resolve_retc_source_files(
        raw_dir,
        sorted(years),
        legacy_cache_dir=cache_dir if allow_legacy_cache else None,
    )
    frames = [_read_year(source_files[year], year) for year in sorted(years)]
    ...
    metadata: dict[str, Any] = {
        ...
        "raw_source": (
            {
                ...
                "manifest": source_snapshot.manifest_path.as_posix(),
                "assets": {
                    str(asset.year): {"path": asset.path, "sha256": asset.sha256}
                    for asset in source_snapshot.assets
                    if asset.year in years
                },
            }
            if source_snapshot is not None
            else {"legacy_cache_fallback": True}
        ),
        "years": years,
        ...
        "communes_with_cd": int((result["hm_cd_kg"] > 0).sum()),
        "communes_with_hg": int((result["hm_hg_kg"] > 0).sum()),
