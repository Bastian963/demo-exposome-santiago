...
The \emph{exposome} is the set of environmental and social exposures that accumulate over a person's life and influence brain health and the risk of cognitive decline and dementia \citep{Livingston2020}. At BrainLat, these exposures usually have to be reconstructed from where each participant lives. This document is a concrete example of how I would build them, and of the data and tools that can keep refining them. Using only \textbf{open data}, I derive more than twenty exposome layers for the \textbf{52 communes of the Metropolitan Region of Santiago} — air quality, industrial heavy metals, green space, artificial light at night, urban heat, precipitation, wildfire, socioeconomic level, healthcare access, and others — integrated into a single master table (52 communes, 261 variables, no gaps). Figure~\ref{fig:fivepanel} shows five representative layers.

Each layer is a self-contained, reproducible notebook and needs no API keys. A static open-data snapshot still misses a great deal, and that is where stronger sources matter. \textbf{Time-domain satellite data} (Sentinel-2 NDVI series, Landsat) and \textbf{computer vision} recover exposures that open maps leave out. Green space is the clearest case: directly from aerial imagery I detect the private gardens and street trees that OpenStreetMap never registers (Fig.~\ref{fig:cv}). Reading satellite archives as time series, to follow change and flag gaps, is the same work I did in time-domain astronomy.

\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{figures/exposome_5panel_santiago.png}
  \caption{\footnotesize Five representative exposome layers for the Metropolitan Region of Santiago, each harmonised to the same 52 communes. The colour scale of every panel and how it is computed are described in Section~\ref{sec:layers}. Diverging scales (socioeconomic level, urban heat) are standardised indices centred at the regional mean; sequential scales are physical magnitudes. The same east--west gradient recurs across panels: the high-income, leafy, cooler east versus the more polluted, hotter, lower-income centre and south-west.}
  \label{fig:fivepanel}
\end{figure}

\section{Key exposome layers: indicator, computation and source}
\label{sec:layers}
...
\item[\textcolor{softpink}{Industrial heavy metals (RETC)}]
\colorbarval{log(Pb$\,+\,$1) from industrial point-source air emissions [kg\,yr$^{-1}$, log scale].}
Lead, arsenic and mercury from industrial facilities declared to the Chilean national pollutant release register (RETC, Ministry of the Environment). Annual facility-level emissions are downloaded for 2015--2022 (CSV + XLSX, no API key), unit-converted to kg/yr, assigned to communes by point-in-polygon spatial join, and averaged across years. Lead is the single best-documented neurotoxin in the Lancet Commission on Dementia 2024: exposure reduces hippocampal volume and accelerates cognitive decline dose-responsively. The most striking finding is that Tiltil, a semi-rural northern commune, concentrates $\approx$99.6\% of all industrial Pb emissions in the Metropolitan Region ($\approx$10\,629 kg/yr), attributable to a known industrial complex; the Spearman correlation between log(Pb) and PM$_{2.5}$ is $\rho\approx{-}0.15$ ($p=0.30$, n.s.), confirming that industrial point-source neurotoxins and combustion-driven particulate pollution are \emph{independent} exposure pathways. \emph{Source:} RETC MMA (\texttt{datosretc.mma.gob.cl}), CC-BY, 2015--2022.
...
Industrial Pb log(kg/yr)      & $+0.22$ & $0.11$   & n.s. \\
\hline
\end{tabular}\par}
\smallskip
{\footnotesize Spearman rank correlations of each exposome layer against the composite socioeconomic index ($n=52$ communes; two-sided $p$-values; $|\rho|\gtrsim0.27$ is significant at $p<0.05$). *** $p<0.001$;\ n.s.\ = not significant. Green space and healthcare show strong socioeconomic gradients; air quality and industrial heavy metals do not --- Pb emissions in Santiago are driven by a single industrial complex in the semi-rural commune of Tiltil, not by deprivation geography.}
...
All code --- one notebook or script per factor, the master integration script (\texttt{scripts/\allowbreak build\_master\_\allowbreak exposome.py}), a conda \texttt{environment.yml}, and the scripts that generate every figure in this document --- is openly available at \url{https://github.com/Bastian963/demo-exposome-santiago}. Every layer is harmonised to the 52 official communes of the Metropolitan Region, computed in a metric projection (EPSG:32719) and exported as both a table (CSV) and a GIS layer (GeoJSON). All inputs are open; most layers require no authentication (RETC, OpenStreetMap, CASEN, INE Census 2017); satellite layers use Google Earth Engine as an optional extension. Each script caches raw responses, so re-runs are offline and deterministic, and the master builder validates a strict one-to-one join (52 rows, 261 variables, no missing values) before writing.
...
\begin{description}[leftmargin=0pt,labelsep=0.5em,itemsep=1.5pt,font=\ttfamily\small]
  \item[notebooks/santiago\_*.ipynb] the factor notebooks and the optional computer-vision example.
  \item[scripts/run\_*.py] config-driven CLI scripts (one per layer); prefer over notebooks for new layers.
  \item[data/processed/*.\{csv,geojson,json\}] per-factor commune indicators as a table, a GIS layer and machine-readable metadata.
  \item[figures/*\_santiago\_*.png \textnormal{,} maps/*\_santiago.html] publication-quality static maps and interactive web maps per factor.
  \item[scripts/build\_master\_exposome.py] joins all layers and writes the master CSV, GeoJSON and metadata ($52\times261$).
  \item[docs/*\_methodology.md] per-layer methodology, data sources, limitations and key findings.
